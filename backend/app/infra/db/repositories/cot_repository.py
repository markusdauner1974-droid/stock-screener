...
_COT_REFRESH_LOCK_ID = 0x434F5452
...
class SqlCotRepository:
    ...
    @contextmanager
    def serialized_refresh(self) -> Iterator[None]:
        bind = self._session.get_bind()
        if bind.dialect.name != "postgresql":
            yield
            return
        with bind.connect() as connection:
            connection.execute(select(func.pg_advisory_lock(_COT_REFRESH_LOCK_ID)))
            try:
                yield
            finally:
                connection.execute(select(func.pg_advisory_unlock(_COT_REFRESH_LOCK_ID)))
