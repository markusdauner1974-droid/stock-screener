...
class TestCalendarCompletenessIsAgeBased:
    ...
    def test_observation_at_max_age_is_complete(self):
        ...
        payload = self._payload(
            event_calendar_as_of_date=(
                date.today() - timedelta(days=EVENT_CALENDAR_MAX_AGE_DAYS)
            ),
            next_earnings_date=None,
        )
    ...
    def test_stale_observation_beyond_window_needs_yahoo_hydration(self):
        from app.services.yahoo_earnings_calendar import EVENT_CALENDAR_MAX_AGE_DAYS
        ...
        payload = self._payload(
            event_calendar_as_of_date=(
                date.today() - timedelta(days=EVENT_CALENDAR_MAX_AGE_DAYS + 1)
            ),
...
class TestHydrationReconcilesCalendarEvidence:
    def test_fresh_yahoo_observation_overrides_stale_persisted_evidence(self):
        ...
        fresh_next = date.today() + timedelta(days=9)
        db = _make_session()()
        _seed_published_run(
            db,
            payload={
        ...
        assert stored["event_calendar_as_of_date"] == date.today()
...
class TestYahooOnlyFieldsStampsCalendar:
    def test_fetch_yahoo_only_fields_includes_fresh_observation(self, monkeypatch):
        earnings_at = pd.Timestamp(datetime.now(UTC).date() + timedelta(days=7))
