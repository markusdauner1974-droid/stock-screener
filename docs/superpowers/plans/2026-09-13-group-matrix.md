...
## Clusters visual correction — 2026-09-14

The user's Finviz screenshot showed a packed circular cloud. The original Clusters implementation reused `MatrixStockPreview`, so it rendered rectangular stock grids inside cards. This was a visual mismatch, despite the two layouts sharing filters and grouping correctly.

Replace only the Clusters preview with deterministic D3 circle packing (`d3-hierarchy` 3.1.2). Retain sector sections, IBD cards and cap columns. Bubble area represents market cap within its card; a 1% floor keeps small stocks visible and unknown caps use equal sizes. The on-screen size legend explains this. Pack up to 64 individual stock buttons per card, with ticker/value labels when they fit, the existing shared inspector, and a complete overflow list. Fixed card heights preserve column virtualization. The shared component serves both live and static pages.

The regression initially failed against the rectangular implementation. Verification covers cap-size ratios, invalid/unknown caps, deterministic non-overlapping geometry, payload immutability, hover/focus inspection, stock drawers and full overflow lists. All **47 frontend tests** pass across nine suites. All **10 production browser tests** pass across live and static builds, including circular geometry in a dense 600-stock fixture, 1440px/390px screenshots and complete 100-stock overflow lists. Desktop and mobile screenshots were visually inspected. Both production builds, targeted ESLint and whitespace checks passed.

The existing 10,000-stock stress fixture remains below 2,000 rendered stock buttons and 131,000 compressed bytes. Layout-switch samples with circle packing: live **4,318.7ms, 242.4ms, 852.1ms**; static **4,464.9ms, 730.5ms, 836.3ms**, Chrome 152.0.7977.83. These three observations per mode do not establish p95; the original 200ms target remains unmet. This correction addresses cluster geometry, without claiming a latency fix.

## Cluster color arrangement — 2026-09-14

The user accepted the circular shape and requested green at the center, neutral between, and red on the perimeter. The previous layout sorted its packing inputs by cap alone, so colors were mixed. Packing order now follows the active color metric descending, while the chosen preview members and cap-based size ratios remain unchanged. Missing values receive a neutral positioning value; the actual values, missing-data treatment and colors are unchanged. The memoized preview repacks when its metric changes.

Three regressions failed before the fix: radial color ordering for daily change and RS, and repositioning when changing the selected metric. All three pass after the fix. Browser checks verify both color modes on the same dense fixture, alongside roundness, non-overlap, inspection, overflow and responsive screenshots.

Final validation: **50 frontend tests** across nine suites and **4 focused production browser checks** across live/static pass. Both production builds, targeted ESLint and whitespace checks pass. Screenshots confirm the green core, neutral middle and red perimeter for daily change and RS. The initial browser metric-switch check used an incomplete accessible name for the Color selector; correcting that test locator required no further application changes. This run did not remeasure the previously documented 200ms stress target.

## Weekly/monthly returns and documentation — 2026-09-14

Added nullable weekly/monthly returns from the selected feature run's stored `perf_week`/`perf_month` fields to the shared live/static payload. Both layouts now offer 1-Day Change, 1-Week Change, 1-Month Change and Stock RS; inspection shows all periods. Weekly/monthly legends use wider percentage bins, and circle packing follows the selected period. Missing values remain missing, with coverage counters and backwards-compatible fallback counts for older static assets. Static snapshots must be republished to gain the new data; no database migration or view-time calculation is required.

Validation: **51 Matrix backend tests**, **16 static combiner tests**, **55 frontend tests**, and **10 production browser tests** across live/static passed. Both production builds passed. Browser checks include weekly/monthly radial ordering and switching layouts while retaining the selected period. Latest stress samples: live **3,411.3ms, 260.5ms, 1,063.6ms**; static **5,397.5ms, 871.9ms, 299.5ms**, 139,330 compressed bytes. These remain observations, not p95 estimates; the 200ms target remains unmet.

Added the detailed [Matrix README](../../GROUP_MATRIX_README.md), main README screenshot sections in the requested order, and a reproducible authenticated local screenshot script. Documentation screenshots use actual published local records: Group rankings dated September 10 and Matrix features dated September 4, 2026, with latest stored metadata disclosed separately.
